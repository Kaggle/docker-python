...
Status ParseScalarArgument(OpKernelContext* ctx, StringPiece argument_name,
  ...
  const ::tensorflow::tstring& tstr = argument_t->scalar<::tensorflow::tstring>()();
  output->assign(tstr.data(), tstr.size());
  return Status::OK();
}
...
class GcsCredentialsOpKernel : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES_OK(ctx, ParseScalarArgument<std::string>(ctx, "json", &json_string));
